...
import { getClient, updateClient } from '@/lib/server/admin-bff';
...
export async function GET(request: NextRequest) {
  try {
    return NextResponse.json(await getClient(getAdminAuthTokenFromRequest(request)));
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await updateClient(payload, getAdminAuthTokenFromRequest(request)));
  } catch (error) {
    if (error instanceof BackendApiError) {
      return jsonError(error.message, error.status, error.code ?? 'validation_error');
    }
