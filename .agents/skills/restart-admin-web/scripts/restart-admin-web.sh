...
APP_DIR="${REPO_ROOT}/apps/admin-web"
RUNTIME_DIR="${REPO_ROOT}/.agents/tmp"
PID_FILE="${RUNTIME_DIR}/admin-web.pid"
LOG_FILE="${RUNTIME_DIR}/admin-web.log"
PORT="3000"
...
terminal_cmd="cd \"${REPO_ROOT}\"; export PATH=\"${HOME}/.local/bin:\$PATH\"; mkdir -p \"${RUNTIME_DIR}\"; : > \"${LOG_FILE}\"; pnpm --dir \"${APP_DIR}\" exec next dev --port \"${PORT}\" 2>&1 | tee -a \"${LOG_FILE}\""
...
if ! osascript \
  ...
  echo "Failed to launch Terminal for admin web restart" >&2
  exit 1
fi
...
for _ in {1..60}; do
  ...
  if [[ -n "${listener_pid}" ]] && /usr/bin/curl -fsS "${HEALTHCHECK_URL}" >/dev/null 2>&1; then
    ...
    echo "Admin web restarted on port ${PORT}"
